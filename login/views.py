        ...
        if form.is_valid():
            ...
            try:
                ...
                if (
                    ...
                    if user.user_active:  # Check if the user is active
                        request.session["user_id"] = user.user_id
                        # Redirect the user to the 'home' view after login
                ...
                messages.error(
                    request, "El usuario no se encuentra registrado"
...
def register(request):
    ...
    if form.is_valid():
        user = User(
            ...
            user_active=True,
            user_inetrest1=form.cleaned_data["user_inetrest1"].af_id,
            user_interest2=form.cleaned_data["user_interest2"].af_id,
...
def comprar_producto(request, prod_id):
    ...
    max_id = Historical.objects.all().aggregate(max_id=Max("hist_id"))["max_id"]
    ...
    message = render_to_string(
        "email_template.html", {"buyer": buyer, "seller": seller, "producto": producto}
    )
    ...
    if user_id is not None:
        ...
        for product in sold_products:
            ...
            previous_score = UserScore.objects.filter(
                user_reviwer=user_reviwer, user_reviwed=user_reviwed
            ).first()
...
def rate_seller(request, seller_id):
    if request.method == "POST":
        ...
        score = UserScore(
            ...
            user_reviwer=user_reviwer,
            user_reviwed=user_reviwed,
...
def products_cards_view(request):
    ...
    products = Product.objects.annotate(likes=Count("userfavoriteproduct")).order_by(
        "-likes"
    )
